...
const MovieDetails = () => {
  ...
  const { data, isLoading: isLoadingGetDetails } = useGetDetailsMovie(id);
  const { data: urlMovie, isLoading: isLoadingGetUrl } = useGetUrlVideos(id);
    ...
    <div className="flex flex-col items-center text-stone-300 w-full h-fit justify-center pb-10">
      ...
      <div className="flex lg:flex-row flex-col sm:w-3/4 w-10/12">
        <div id="movie-details" className="lg:w-3/5 w-full lg:mb-0 mb-10">
          {(!movieDetailsData && isLoadingGetDetails) || isLoadingGetUrl ? (
            <MovieDetailsLoading />
          ) : (
            <>
              <h2>{`${
              ...
              <a
                href={`https://www.youtube.com/watch?v=${urlMovie?.key}`}
                target="_blank"
              >
